...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Straks database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Straks datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Straks datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Straks 0.7 datadir."
    ;;
  3)
    echo "Detected Straks pre-0.8 datadir."
    ;;
  4)
    echo "Detected Straks 0.8 datadir."
    ;;
esac
