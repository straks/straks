...
*Setup instructions for a gitian build of Straks using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Straks
...
- [Building Straks](#building-straks)
...
Clone the git repositories for straks and gitian,
...
In [doc/release-process.md](release-process.md) in the straks repository under 'Fetch and build inputs'.
...
To build Straks (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the straks repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/straks/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/straks/straks
...
For example:
```bash
URL=https://github.com/laanwj/straks.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit straks=${COMMIT} --url straks=${URL} ../straks/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit straks=${COMMIT} --url straks=${URL} ../straks/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/straks-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/straks-build.assert
```
...
[straks/gitian.sigs](https://github.com/straks/gitian.sigs/) repository, or
