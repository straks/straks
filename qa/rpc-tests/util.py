...
# Copyright (c) 2014-2017 The Straks developers
...
def initialize_chain(test_dir):
    ...
    4 wallets.
    straksd and straks-cli must be in search path.
    """
    ...
    if not os.path.isdir(os.path.join("cache", "node0")):
        ...
        for i in range(4):
            ...
            os.makedirs(datadir)
            with open(os.path.join(datadir, "straks.conf"), 'w') as f:
                f.write("regtest=1\n");
                f.write("rpcuser=rt\n");
                f.write("rpcpassword=rt\n");
                f.write("port="+str(START_P2P_PORT+i)+"\n");
                f.write("rpcport="+str(START_RPC_PORT+i)+"\n");
            args = [ "straksd", "-keypool=1", "-datadir="+datadir ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(START_P2P_PORT))
            straksd_processes.append(subprocess.Popen(args))
            subprocess.check_call([ "straks-cli", "-datadir="+datadir,
...
def start_nodes(num_nodes, dir):
    # Start straksds, and wait for RPC interface to be up and running:
    devnull = open("/dev/null", "w+")
    for i in range(num_nodes):
        datadir = os.path.join(dir, "node"+str(i))
        args = [ "straksd", "-datadir="+datadir ]
        straksd_processes.append(subprocess.Popen(args))
        subprocess.check_call([ "straks-cli", "-datadir="+datadir,
                                  "-rpcwait", "getblockcount"], stdout=devnull)
    devnull.close()
...
def wait_straksds():
    # Wait for all straksds to cleanly exit
    for straksd in straksd_processes:
        straksd.wait()
    del straksd_processes[:]
