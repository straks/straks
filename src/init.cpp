 ...
 * between straksd, and straks-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), STRAKS_CONF_FILENAME));
    if (mode == HMM_STRAKSD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), STRAKS_PID_FILENAME));
#endif
...
/** Sanity checks
 *  Ensure that Straks is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single straks process is using the data directory.
    ...
    try {
       ...
       // Wait maximum 10 seconds if an old wallet is still running. Avoids lockup during restart
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Straks is probably already running."), strDataDir));
        if (probeOnly) {
            lock.unlock();
        }
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", STRAKS_CONF_FILENAME)).string());
	...
	if (mapMultiArgs.count("-masternodepaymentskey")) // masternode payments priv key
    {
        if (!masternodePayments.SetPrivKey(GetArg("-masternodepaymentskey", ""))){
			...
			//LogPrintf("Masternode: Create new key %s", CStraksSecret(secret).ToString());
   ...
   //if (!fDisableWallet) {
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Straks was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Straks with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty()) {
        return InitError(_("Straks was not built with wallet support. Set -mineraddress, or rebuild Straks with wallet support."));
    }
 #endif // !ENABLE_WALLET
    ...
    if (mapMultiArgs.count("-mineraddress")) {
        ...
        if (pwalletMain) {
            ...
            CStraksAddress addr(GetArg("-mineraddress",""));
            CKeyID keyID;
            addr.GetKeyID(keyID);
