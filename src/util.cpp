...
const char * const STRAKS_CONF_FILENAME = "straks.conf";
const char * const STRAKS_PID_FILENAME = "straks.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
				...
				/**TODO-- */
				// "straks" is a composite category enabling all Straks-related debug output
				if(ptrCategory->count(string("straks"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\straks
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\straks
    // Mac: ~/Library/Application Support/straks
    // Unix: ~/.straks
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "straks";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/straks";
#else
    // Unix
    return pathRet / ".straks";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
		/**TODO-- */
        // Create empty straks.conf if it does not excist
        //FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
        //if (configFile != NULL)
            //fclose(configFile);
        return; // Nothing to read, so just return //return; // No straks.conf file is OK
    }
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override straks.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", STRAKS_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Straks Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Straks Core") == std::string::npos) {
