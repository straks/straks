...
#if defined(NDEBUG)
# error "Straks cannot be compiled without assertions."
#endif

/**
 * Global state
 */
#define PRE_SUBSIDY_HEIGHT 10080
...
const std::string strMessageMagic = "Straks Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // [squbs] for Signatum investor swap and dev allocation
    // (instamine required for supply tracking purposes)
    if (nHeight == 1)
        nSubsidy = 41250000 * COIN;
    // further protection 7 days, 10080 blocks
    else if (nHeight > 1 && nHeight <= PRE_SUBSIDY_HEIGHT)
        nSubsidy = 0;
    //hard supply limit after ~20 years
    else if (nHeight > 10885080)
        nSubsidy = 0;
